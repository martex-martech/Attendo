...
const StatCardSkeleton: React.FC = () => (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-lg animate-pulse">
        ...
        <div>
            <div className="h-4 w-3/4 bg-slate-200 dark:bg-slate-700 rounded-md mt-3"></div>
            <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-700 rounded-full mt-2"></div>
            <div className="h-3 w-1/2 bg-slate-200 dark:bg-slate-700 rounded-md mt-2"></div>
        </div>
    </div>
);
...
const ChartSkeleton: React.FC = () => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg animate-pulse">
        ...
        <div className="w-full h-[250px] bg-slate-200 dark:bg-slate-700 rounded-md"></div>
        <div className="flex justify-center items-center space-x-6 mt-4">
            <div className="flex items-center">
                <div className="w-3 h-3 bg-slate-200 dark:bg-slate-700 rounded-full mr-2"></div>
        ...
        <td className="p-3">
            <div className="flex items-center">
                ...
                <div>
                    ...
                    <div className="h-3 w-16 bg-slate-200 dark:bg-slate-700 rounded mt-1"></div>
                </div>
            </div>
        </td>
...
const ReportStatCard: React.FC<{ data: ReportStatCardData }> = ({ data }) => {
    return (
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-lg flex flex-col justify-between">
            <div className="flex items-start justify-between">
            ...
            <div>
                <p className="text-gray-600 dark:text-gray-400 mt-3">{data.title}</p>
...
const AttendanceChart: React.FC<{
    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center"><Icon name="analytics" className="mr-2 text-red-500"/>Attendance</h2>
        ...
        <div className="w-full overflow-x-auto">
            <svg viewBox={`0 0 ${width} ${height}`} className="min-w-[800px] w-full">
                ...
                {[...Array(5)].map((_, i) => {
                    ...
                    return (
                    <g key={val}>
                        <line x1={padding} y1={scaleY(val)} x2={width-padding} y2={scaleY(val)} stroke="currentColor" className="text-gray-200 dark:text-slate-700" strokeWidth="1" />
                        <text x={padding - 10} y={scaleY(val) + 5} textAnchor="end" className="text-xs text-gray-500 dark:text-gray-400 fill-current">{val}</text>
                    </g>
                )})}
                
                {/* X-axis labels */}
                {data.labels.map((label, i) => (
                    <text key={label} x={scaleX(i)} y={height - padding + 20} textAnchor="middle" className="text-xs text-gray-500 dark:text-gray-400 fill-current">{label}</text>
                ))}
        ...
        <div className="flex justify-center items-center space-x-6 mt-4">
            <div className="flex items-center text-sm"><span className="w-3 h-3 bg-green-500 rounded-full mr-2"></span><span className="text-gray-600 dark:text-gray-300">Present</span></div>
            <div className="flex items-center text-sm"><span className="w-3 h-3 bg-pink-500 rounded-full mr-2"></span><span className="text-gray-600 dark:text-gray-300">Absent</span></div>
        </div>
      </div>
    );
};
...
const ReportsPage: React.FC<{ employees: Employee[] }> = ({ employees }) => {
    ...
    const getStatusClass = (status: 'Present' | 'Absent' | 'Late') => {
        if (status === 'Present') return 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300';
        if (status === 'Late') return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300';
        return 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300';
    };

    const getProductionBadgeClass = (hours: number) => {
        if (hours > 9) return "bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300";
        if (hours >= 8) return "bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300";
        return "bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300";
    };
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
                 <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 flex-wrap">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Employee Attendance</h2>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 dark:bg-slate-900/50">
                            <tr>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Name</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Date</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Check In</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Status</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Check Out</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Break</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Late</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Overtime</th>
                                <th className="p-3 text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wider">Production</th>
                            </tr>
                        </thead>
                        <tbody>
                             {isLoading ? (
                                ...
                                <tr key={rec.id} className="border-b border-gray-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.date}</td>
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.checkIn}</td>
                                    ...
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.checkOut}</td>
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.break}</td>
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.late}</td>
                                    <td className="p-3 text-gray-600 dark:text-gray-300">{rec.overtime}</td>
                                    <td className="p-3">
